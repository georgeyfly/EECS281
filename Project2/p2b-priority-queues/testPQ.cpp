...
void testHiddenData() {
    ...
    struct HiddenDataComp {
        bool operator()(HiddenData const& a, HiddenData const& b) const {
    ...
    // PQ< HiddenData > eecsPQ (HiddenDataComp);
    // // Eecs281PQ<HiddenData>& eecsPQ = pq;
...
int main() {
    ...
    int choice {};
    std::cin >> choice;
    PQType const pqType = types.at(static_cast<size_t>(choice));
